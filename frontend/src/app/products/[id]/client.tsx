...
import { Shield, Truck, RotateCcw, ChevronRight, Heart, ShoppingCart } from "lucide-react";
  ...
  return (
    <main className="min-h-screen bg-background text-foreground pt-32 pb-24 overflow-hidden relative">
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        {/* Breadcrumbs */}
        <nav className="flex items-center gap-2 text-sm text-foreground/70 mb-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-4 mt-4">
              {[1, 2, 3].map((i) => (
                <div key={i} className="relative aspect-square rounded-2xl bg-foreground/5 overflow-hidden border border-foreground/10 opacity-70 hover:opacity-100 cursor-pointer transition-opacity group">
                  <Image src={product.image_url || ""} alt="" fill sizes="(max-width: 768px) 33vw, 15vw" className="object-cover group-hover:scale-110 transition-transform duration-500" />
                </div>
              ))}
            </div>
          </motion.div>
