...
export const useAppStore = create<AppState>()(
  persist(
    (set) => ({
      ...
      addToCart: (product, quantity = 1, variant = "") => set((state) => {
        // We match by product id AND variant to allow different variants in cart
        const existingId = state.cartItems.findIndex(item => item.id === product.id && item.variant === variant);
        let newItems = [...state.cartItems];
        
        if (existingId >= 0) {
          newItems[existingId].quantity += quantity;
        } else {
          newItems.push({ ...product, quantity, selected: true, variant });
        }
